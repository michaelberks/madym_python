...
#-----------------------------------------------------------------------------
def read_jim_roi(roi_path, roi_dims, vox_size):
    ...
    for i_slice in range(num_slices):
        ...
        if 'deleted' not in slice_info_i:
    ...
    for i_z in range(n_z):
        ...
        for contour, internal in zip(contours,internal):
            if contour.shape[0] > min_contour:
                ...
                contour[:,0] -= n_x/2 
                contour[:,1] -= n_y/2
                contour[:,0] *= vox_size[0]
                contour[:,1] *= vox_size[1]
...
#-----------------------------------------------------------------------------
def write_slice_pts(fid, contour):
    print("  Begin Shape", file=fid)
    print(f"    Points={len(contour)}", file=fid)
    for xy in contour:
        print(f"    X={xy[0]:4.3f}; Y={xy[1]:4.3f}", file=fid)
